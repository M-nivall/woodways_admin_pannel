...
if ($_SERVER['REQUEST_METHOD'] =='POST') {
    ...
    if (empty($mpesaCode)) {
        ...
        WHERE oi.order_id='$orderID' AND oi.type = 'order'";
        $query = mysqli_query($con, $select);
        while ($row = mysqli_fetch_array($query)) {
            $price = $row['price'];  // item price

            // update order items price
            $update = "UPDATE order_items SET item_price='$price' WHERE order_id='$orderID' AND type = 'order'";
            if (mysqli_query($con, $update)) {
                    ...
                    if (mysqli_query($con, $update)) {
                        ...
                        $update = "UPDATE order_items SET item_status='1' WHERE order_id='$orderID' AND type = 'order'";
                        mysqli_query($con, $update);
                        ...
                        $select = "SELECT stock_id,quantity,item_price FROM order_items WHERE order_id='$orderID' AND type = 'order'";
                        $query = mysqli_query($con, $select);
                        while ($row = mysqli_fetch_array($query)) {
