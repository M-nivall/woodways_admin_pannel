...
// check if item exist in client shopping cart
        ...
        if($get=mysqli_num_rows($records) >0){
            $response["status"] = 0;
            $response["message"] ='You have Already requested for this service';
            echo json_encode($response);
        }else{
            // check if client has an active order
            $select="SELECT * FROM bookings WHERE client_id='$clientID' AND order_status='0'";
            $record=mysqli_query($con,$select);
            if ($row=mysqli_num_rows($record) < 1 ){
             ...
             VALUES ('$clientID','$orderNo', '$productID', '$qty','service')";
                if (mysqli_query($con,$insrt)) {
                    $response["status"] = 1;
                    $response["message"] ='Service Has been Requested successfully, Please Check Your Service Request List';
                    echo json_encode($response);
