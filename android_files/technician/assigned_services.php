...
$select = "
    SELECT 
        ...
        a.assign_status
    FROM bookings o
    INNER JOIN assigned_services a 
        ON o.order_id = a.order_id
    INNER JOIN tools_requests t 
        ON o.order_id = t.order_id    
    INNER JOIN clients c2 
        ON o.client_id = c2.client_id
    WHERE 
        a.emp_id = '$staffID'
        AND o.order_status = '5'
        AND a.assign_status = 'Assigned'
        AND t.request_state = 'Approved'
...
if (mysqli_num_rows($query) > 0) {
    $results = [];
    $results['status'] = "1";
    $results['message'] = "Assigned Duties Found";
    $results['details'] = [];
    ...
    while ($row = mysqli_fetch_array($query)) {
        ...
        $temp['orderStatus'] =  $row['assign_status'];
