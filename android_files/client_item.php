...
$select = "SELECT * FROM orders WHERE order_id='$orderID'";
  ...
  if(mysqli_num_rows($query)>0){
      ...
      $select = "SELECT s.product_name,s.stock_id,s.price,oi.item_id,oi.quantity,oi.type FROM stock s 
  INNER JOIN order_items oi on s.stock_id = oi.stock_id  WHERE oi.order_id='$orderID' AND oi.type = 'order'";
      $query=mysqli_query($con,$select);
